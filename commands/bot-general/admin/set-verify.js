...
module.exports = {
    ...
    async execute(interaction) {
        ...
        // FIX 1: Nombres de variables idénticos a las columnas de MariaDB
        let config = {
        ...
        const getComponents = () => {
            ...
            if (step === 1) {
                rows.push(new ActionRowBuilder().addComponents(
                    new ChannelSelectMenuBuilder().setCustomId('select_channels').setPlaceholder('Seleccionar canales...').addChannelTypes(ChannelType.GuildText).setMaxValues(3)
                ));
                rows.push(new ActionRowBuilder().addComponents(
                ...
                rows.push(new ActionRowBuilder().addComponents(
                    new RoleSelectMenuBuilder().setCustomId('select_roles').setPlaceholder('Seleccionar roles...').setMaxValues(3)
                ));
                rows.push(new ActionRowBuilder().addComponents(
                ...
                rows.push(new ActionRowBuilder().addComponents(
                    new ChannelSelectMenuBuilder().setCustomId('select_optional').setPlaceholder('Canales opcionales...').addChannelTypes(ChannelType.GuildText).setMaxValues(2)
                ));
                rows.push(new ActionRowBuilder().addComponents(
        ...
        const response = await interaction.reply({ embeds: [getEmbed()], components: getComponents(), flags: [MessageFlags.Ephemeral] });
        ...
        collector.on('collect', async i => {
            ...
            if (i.customId === 'finish') {
                try {
                    ...
                    // FIX 2: Usamos interaction.client en lugar de client
                    logError(interaction.client, err, "Finalizar Setup Wizard", guild.id);
                    return i.update({ content: "❌ Error al guardar. Revisá el canal de debug.", embeds: [], components: [] });
                }
            }
